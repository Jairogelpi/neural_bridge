    ...
    static async refineKnowledge(sessionId: string, interactionResult: string): Promise<void> {
        ...
        for (const crystalId of session.active_crystal_ids) {
            ...
            try {
                ...
                const updatedConstraints = JSON.parse(response.content.match(/\[[\s\S]*\]/)?.[0] || '[]');
                ...
                if (updatedConstraints.length > 0) {
                    ...
                    const childCrystal = await SynapticBinder.fractalize(crystal, JSON.stringify(updatedConstraints), 'REFINEMENT');

                    // Apply the new constraints to the child
                    childCrystal.constraints = updatedConstraints;
