...
export class SemanticHasher {
     ...
     */
    static hash(features: SemanticFeature[]): string {
     ...
     */
    static isParaphrase(text1: string, text2: string, threshold: number = 0.7): boolean {
        ...
        const byteHash1 = crypto.createHash('sha256').update(text1).digest('hex');
        const byteHash2 = crypto.createHash('sha256').update(text2).digest('hex');
...
export class SMTBuilder {
     ...
     */
    build(text: string): SemanticMerkleTree {
        const treeId = `smt_${crypto.randomBytes(8).toString('hex')}`;
        ...
        for (const [type, groupFeatures] of Object.entries(featureGroups)) {
            const node = this.createNode(groupFeatures, 0);
            this.nodes.set(node.id, node);
            leafNodes.push(node);
        }
        
        // Build tree bottom-up
        const root = this.buildTreeLevel(leafNodes, 1);
        
        // Extract claims for audit trail
        this.extractClaims(features, text);
        
        // Calculate document hashes
        const byteHash = crypto.createHash('sha256').update(text).digest('hex');
        const semanticHash = SemanticHasher.hash(features);
    ...
    private createNode(features: SemanticFeature[], depth: number): SemanticNode {
    ...
    private buildTreeLevel(nodes: SemanticNode[], depth: number): SemanticNode {
        if (nodes.length === 0) {
            return this.createNode([], depth);
        }
        ...
        for (let i = 0; i < nodes.length; i += 2) {
            ...
            const parent = this.createNode(combinedFeatures, depth);
        ...
        return this.buildTreeLevel(parentNodes, depth + 1);
    }
    
    private extractClaims(features: SemanticFeature[], text: string): void {
        ...
        for (const claim of claimFeatures) {
            this.claims.push({
                ...
                semantic_hash: crypto.createHash('sha256').update(claim.canonical).digest('hex'),
...
export class SMTRuntime {
     ...
     */
    static build(text: string): SemanticMerkleTree {
     ...
     */
    static compare(text1: string, text2: string): SemanticComparisonResult {
        const smt1 = this.build(text1);
        const smt2 = this.build(text2);
        
        const features1 = SemanticExtractor.extract(text1);
        const features2 = SemanticExtractor.extract(text2);
        
        const similarity = SemanticHasher.similarity(features1, features2);
        const isParaphrase = SemanticHasher.isParaphrase(text1, text2);
        ...
        const bytesSame = crypto.createHash('sha256').update(text1).digest('hex') ===
                         crypto.createHash('sha256').update(text2).digest('hex');
        ...
        return {
            ...
            comparison_proof: {
                ...
                comparison_hash: crypto.createHash('sha256')
                    .update(smt1.document.semantic_hash + smt2.document.semantic_hash)
                    .digest('hex'),
     ...
     */
    static verifyClaim(smt: SemanticMerkleTree, claim: string): {
        found: boolean;
        ...
        const claimHash = SemanticHasher.hash(claimFeatures);
